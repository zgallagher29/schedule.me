    ...
    /** TODO: we want to make this compatible with random screen sizes e0-tc */
    ...
    $('.timepicker').pickatime({
        default: '08:00AM', // Set default time: 'now', '1:30AM', '16:30'
    ...
    $('.btn').click(function(){
        if ($(this).hasClass("grey lighten-1")){
            ...
            $(this).addClass(defaultColor + " selected");
        }
        else{
            $(this).removeClass(defaultColor + " selected");
    ...
    function makeClassFromForm(){
    var name = $('#addclass_name').val();
    var location = $('#addclass_location').val();
    var start_time = $('#addclass_starttime').val();
    var end_time = $('#addclass_endtime').val();
    var days = [];
    var elements = document.getElementsByClassName("selected");

    for (var i = 0; i < elements.length; i++){
        switch(elements.item(i).textContent){
            case "Monday":
                days.push(0);
                break;
            case "Tuesday":
    ...
    var classInfo = {"name": name,"timeString": start_time + " - " + end_time, "time": [start_time.replace(":",''),end_time.replace(":","")],"days":days,"location": location};    
    
    console.log(classInfo);
    for (var day = 0; day < 5; day++){
            if (classInfo.days.includes(day)){
                makeCard(classInfo,day);
            }   
    }
